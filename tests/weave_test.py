...
class WeaveTests(unittest.TestCase):
    ...
    def test_weavable_services_are_set(self):
        self.assertCountEqual(self.woven.weavable['zipcodes@web-service'].services, 
    ...
    def test_required_shared_properties_are_set(self):
        self.assertCountEqual(self.woven.required['shared-property@https-url'].services, 
                            [Reference(service='zipcodes', path=['requires', 'properties', 'shared-property']),
                            Reference(service='geo', path=['requires', 'properties', 'shared-property'])])
        
    def test_required_unique_properties_are_set(self):
        self.assertCountEqual(self.woven.required['geoservice-requires-properties-geo-url@https-url'].services, 
